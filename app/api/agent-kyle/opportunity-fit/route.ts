...
import { generateOpportunityFit } from "@/lib/agent-kyle/engine";
...
import { opportunityFitInputSchema } from "@/lib/agent-kyle/types";
...
export async function POST(request: NextRequest) {
  ...
  const parsed = opportunityFitInputSchema.safeParse(payload);
  if (!parsed.success) {
    const response = NextResponse.json(
      {
        error: "Invalid opportunity fit payload.",
  ...
  try {
    const result = await generateOpportunityFit(parsed.data);
