...
3. Add a strong random `PLAYGROUND_RATE_LIMIT_SECRET` to Production, Preview, and Development as appropriate.
...
When `DATABASE_URL` is absent or the schema cannot be queried, the API returns an honest unavailable mode and the browser shows scores from this device only. Global writes additionally require `PLAYGROUND_RATE_LIMIT_SECRET`.
...
- The request IP and user agent are HMAC-hashed with `PLAYGROUND_RATE_LIMIT_SECRET`; raw network identifiers are not stored.
