...
export interface ClothElements {
  ...
  /** Paints a standard-canvas texture when html-in-canvas is unavailable. */
  paintFallback?: ClothFallbackPainter;
}

export type ClothFallbackPainter = (
...
// The upstream demo uses 96 segments. A 48-segment mesh keeps the same
// shader/physics behavior while sharing the page comfortably with Particle Scroll.
const SEG = 48;
const NODES = SEG + 1;
const DT = 1 / 120;
const WAVE_SPEED = 30;
...
export function createCloth(
  ...
  const { source, content, output, paintFallback } = elements;
  ...
  const canvasFallback = Boolean(sourceCtx && paintFallback);
  const canRender = htmlInCanvas || canvasFallback;
  ...
  capture = () => {
    ...
    try {
      ...
      if (htmlInCanvas) {
        ...
        paintFallback!(sourceCtx!, source.width, source.height);
      }
      gl!.bindTexture(gl!.TEXTURE_2D, contentTexture);
      gl!.texImage2D(
        gl!.TEXTURE_2D,
  ...
  function syncCanvasSize() {
    ...
    if (canRender) {
      const cssWidth = Math.max(
        1,
        Math.round(htmlInCanvas ? source.clientWidth : wrapper.clientWidth),
      );
      const cssHeight = Math.max(
        1,
        Math.round(htmlInCanvas ? source.clientHeight : wrapper.clientHeight),
      );
